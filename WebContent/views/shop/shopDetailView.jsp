...
<body>
	<!-- 
      해야할 것 : 
         ...
         1) DB구축 후 hidden으로 값 받아오는 input Tag들 만들기  
         2) 배송정보 칸의 안내창(물음표) 이미지 hover이벤트(시간남으면)
   -->
	<!-- 상세 페이지 전체 감싸는 div -->
	<div id="Index" class="flex column">
		...
		<div id="goTop">
			...
			<div id="recent_prd">
			<% for(int i=1; i<=3; i++){ %>
				<img alt="최근 본 상품" src="../../img/shopImg/photo<%=  i  %>.jpg">
				<p>상품명</p>
				<% } %>
			</div>
			<a href="#main"><span>TOP</span></a> 
			<a href="#"><span>CART</span></a>
				...
				<form name="itemForm" method="post" id="itemForm">
					<input type="hidden" id="itemId" name="itemId" value=""> <input
						type="hidden" id="brandId" name="brandId" value=""> <input
						type="hidden" id="cId" name="cId" value=""> <input
						type="hidden" id="price" name="price" value=""> <input
						type="hidden" id="stock" name="stock" value=""> <input
						type="hidden" id="sellcount" name="sellcount" value="">
				</form>
				<div id="content_top">
					<!-- 사진 영역 -->
					<div id="ct_col_lft">
					...
					<div id="ct_col_rgt">
						...
						<table>
							...
							<tbody id="itemTableTbody">
								<tr>
									<td>배송정보</td>
									<td><span>택배배송</span> 평균 2일 이내 배송(토,일,공휴일제외) <img
										alt="안내창"
					...
					<div id="cb_info">
						...
						<div class="cb_info_inner">
							...
							<div class="text_box">
								...
								주문시 입력한 입금자명과 실제 입금자의 성명이 반드시 일치하여야 하며, 7일 이내로 입금을 하셔야 하며 입금되지
								않은 주문은 자동취소 됩니다.<br>
							</div>
						</div>
						<div class="cb_info_inner">
							...
							<br>
							<div class="text_box">
								배송 방법: 택배<br> 배송 지역: 전국지역<br> 배송 비용: 3,000원<br>
								배송 기간: 2일 ~ 5일<br> 배송 안내: 산간 벽지나 도서지방은 별도의 추가금액을 지불하셔야 하는
								경우가 있습니다<br> 고객님께서 주문하신 상품은 입금 확인 후 배송해 드립니다. 다만, 상품 종류에
						...
						<div class="cb_info_inner">
							...
							<div class="text_box">
								...
								멸실 또는 훼손된 경우. 단, 상품의 내용을 확인하기 위하여 포장 등을 훼손한 경우는 제외 <br>-
								포장된 상품의 일부를 소모하거나 훼손되어 상품가치가 하락된 경우(주문하신 상품 외 기타 동봉된 내용물은 해당사항
								없음) <br>- 고객님의 사용 또는 일부 소비에 의하여 상품의 가치가 현저히 감소한 경우 <br>-
								시간의 경과에 의하여 재판매가 곤란할 정도로 상품등의 가치가 현저히 감소한 경우 <br>- 복제가 가능한
								상품등의 포장을 훼손한 경우 (자세한 내용은 카카오톡 1:1 / E-MAIL / 전화 상담을 이용해 주시기
								바랍니다.) <br> <br>※ 고객님의 마음이 바뀌어 교환, 반품을 하실 경우 상품반송 비용은
								고객님께서 부담하셔야 합니다.
							</div>
						</div>
					</div>
	...
	<script>
		...
		function cssResize() {
			if (windowWidth <= 900) {
				...
				$('#goTop').css('width','10vw');
				$('#goTop a::after').css('content', '');
			} else {
